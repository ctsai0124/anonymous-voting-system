...
### 7.3 紙本選票「除當然委員N人外，應選委員M人」算式對不起來（3+8≠9），考核會的委員總額更是完全不跟著使用者輸入變動

**問題**：使用者用「範例國民小學（示範資料）」把教評會「委員總額」滑桿從預設 11 調成 9 後，點「產製紙本選票」，說明欄第三項印出「本會置委員 9 人，除當然委員 3 人外，應票選委員 8 人」——3+8=11，不等於委員總額 9，算式自相矛盾。回報過程中進一步發現考核會（教師成績考核委員會）更嚴重：把「委員會類型」切成考核會、調整「考核會委員總額」滑桿（例如改成 9），印出的選票內容裡「委員總額」文字完全不會變，卡在另一個跟滑桿無關的數字，滑桿調多少都沒用。

**根因**：共用試算函式 `computeCommitteeNumbers()`（選票、簽收清冊、計票單、法規提示都呼叫這一個）裡，兩種委員會類型各自的資料流向都寫反了：
- 教評會：`委員總額`(`committee_total_n`)是管理者輸入的權威數字沒錯，但「應選委員」數字卻不是從`總額－當然委員`反推，而是另外去讀一個完全獨立的欄位（「圈選上限人次」`max_selectable`，預設值8），這個欄位只有管理者手動點「一鍵帶入」按鈕或自己手動改欄位才會更新，光是拖動委員總額滑桿完全不會同步過去，兩個欄位各自為政，數字自然對不上。
- 考核會：「委員總額」文字顯示的數字根本不是讀`pr_committee_total_n`滑桿，而是拿「圈選上限人次」欄位(`max_selectable_pr`或`max_selectable`)＋名冊裡實際標記的當然委員人數直接加總反推出來的——滑桿的值從頭到尾沒被讀取過，難怪調滑桿完全沒反應。

**修正內容**（`~/anonymous-voting-system/index.html`，只動 `computeCommitteeNumbers()` 這一個函式，選票/簽收清冊/計票單/法規提示等下游全部共用同一份，改一處全部一起修好）：
- 兩種類型都改成「委員總額（管理者輸入的權威數字：教評會讀`committee_total_n`，考核會讀`pr_committee_total_n`）－ 當然委員（教評會依「學校已成立教師會」勾選框算2或3人；考核會依名冊實際標記人數）＝ 應選委員」這個法定方向去算，不再反過來拼湊委員總額。
- 算出正確的應選委員數字後，同時同步寫回`max_selectable`／`max_selectable_pr`欄位本身（`syncMaxSelectable()`），避免「選票印的是修正後數字、但『發布線上投票』讀到的還是欄位裡的舊值」這種新的分裂——這兩個地方以後永遠讀同一個已經校正過的數字。
- 備份檔：`index.html.bak-20260907-080623`（改動前）。

**驗證方式**：(1) 用 in-app 瀏覽器實際打開 https://election.leotsai.me 、載入範例名冊、把委員總額調成9，點產製紙本選票，修正前重現了回報的「9人／3人／8人」矛盾文字，也重現了考核會滑桿完全不跟進的狀況（滑桿調9，印出的委員總額卻是另一個跟滑桿無關的數字）。(2) 部署修正後，用 curl 直接抓正式站當下實際回應的 `index.html` 原始碼、抽出`computeCommitteeNumbers`函式本體，在 Node.js 裡用假的`document`/`parsedVotersList`物件實際執行這個函式（不是重看程式碼猜結果，是真的跑這個從正式站抓下來的函式），對教評會測了委員總額5／6／9／11／14／19人×有無教師會代表共8組、考核會測了委員總額5／9／13／17人×當然委員0～5人共6組、同時辦理模式測了教評會與考核會互不干擾各1組，每一組都獨立手算「當然委員＋應選委員應該等於委員總額」以及各項法定門檻（未兼行政下限、性別下限）的期望值再跟函式實際回傳值比對，全數16組完全通過，包含回報的原始案例（教評會、總額9）確認變成「當然委員3人＋應選委員6人＝9人」。(3) 另外用同一份測試模擬同一個瀏覽器session裡連續多次調整委員總額（不重新整理頁面），確認每次都正確跟進，排除「改了但沒有即時反映」這種殘留狀態疑慮。

**已知邊界情況（這次沒有動，但如實記錄）**：考核會有個第9條但書——委員總額降到9人以下（可低至5人）時，當然委員法定上限是「至多2人」，畫面上已經有另一個獨立的紅字警告（`pr-proviso-notice`）在提醒管理者這個狀態下如果當然委員職稱清單還留超過2個要自己清掉。如果管理者無視這個警告、硬是在委員總額只有5人的情況下，名冊還留著5位當然委員標記，`computeCommitteeNumbers()`目前算出來的應選委員會被下限鎖在1人（`Math.max(1, ...)`），此時當然委員＋應選委員（5+1=6）還是會跟委員總額（5）對不上——但這是管理者違反畫面上已經在警告的「但書當然委員至多2人」規則所導致的資料本身就自相矛盾，不是這次修的這個bug的同類問題，正常照著畫面警告清理當然委員清單就不會遇到。
